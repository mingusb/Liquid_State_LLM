...
This is a self-improving harness for LLMs (currently tested on OpenAI Codex 5.3 low/xhigh and Codex Spark). It is based on the core idea of heirarchical rubrics, such that there is a meta-rubric that is used to score the rubric, and a meta-meta-rubric to score the meta-rubric, and so on. Rubric dimensions correspond to N roles in a company, with each role contributing a number of dimensions to each layer of rubric. Rubrics are scored by judges. Additionally, the entire chain of rubrics is repeatedly destabilized and redeveloped. The prompts, such as AGENTS.md, encourage the LLM to act as a Liquid State Machine on the Edge of Chaos, which is thought to further enhance generalization, resulting in so-called extrageneralization (Maass, 2000). This harness is capable of self-improvement, which is how it has been developed thus far. It is not particularly efficient or effective, and can only be used with a Pro-tier ($200/month) subscription to GPT. Benefits include that it is relatively well developed, and easy to use. Additionally, large numbers of sub-agent swarms have been repeatedly deployed to seek out and fix bugs in the project. However, there are no tests.
